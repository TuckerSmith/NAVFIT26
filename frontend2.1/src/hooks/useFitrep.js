...
export default function useFitrep(activeSqliteDb) {
    ...
    const handlePDFExport = async () => {
        // Feed the most recently saved data directly into the PDF generator
        const result = await window.api.exportPDF(selectedReport);
        if (result.success) {
            triggerNotification("Success", `PDF exported to your Documents folder!`, false);
        }
    };
    ...
    const handleACCDBExport = async () => {
        ...
        if (result.success) {
            ...
            triggerNotification("Error", "Failed to generate ACCDB.", true);
        }
    };

    const getError = (field) => {
        const validator = validators[field];
        if (!validator) return { isError: false, note: "" };
    ...
    return {
        ...
        handleACCDBExport, isSaved, hasUnsavedChanges, getError
    };
}